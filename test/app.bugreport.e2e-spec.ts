...
describe('BugReportController (e2e)', () => {
  ...
  describe('POST ---> /bug_report', () => {
    it('Should be able to create a bug report', async () => {
        ...
        const { status, body } = await request(app.getHttpServer()).post('/bug_report').send(bugReportCreateData).set('Authorization', `Bearer ${token}`)
        report = body
    ...
    it('Should not be able to create a bug report without token', async () => {
        const { status, body } = await request(app.getHttpServer()).post('/bug_report').send(bugReportCreateData)
  ...
  describe('GET ---> /bug_report', () => {
    it('Should be able to list all bug reports', async () => {
        const { status, body } = await request(app.getHttpServer()).get('/bug_report').set('Authorization', `Bearer ${token}`)
    ...
    it('Should not be able to list all bug reports without token', async () => {
        const { status, body } = await request(app.getHttpServer()).get('/bug_report')
  ...
  describe('DELETE ---> /promotions/:id', () => {
    it('Should be able to delete a bug report', async () => {
        const { status } = await request(app.getHttpServer()).delete(`/bug_report/${report.id}`).set('Authorization', `Bearer ${token}`)

        expect(status).toBe(204)
    })

    it('Should be able to delete a bug report without token', async () => {
        const { status, body } = await request(app.getHttpServer()).delete(`/bug_report/${report.id}`)
